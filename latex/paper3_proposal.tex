...
\textbf{The first non-Gaussian rung.} $M_{\mathrm{recon}}$ is the quadratic rung
of a local expansion. In the natural (exponential-family) coordinates of a
reference chart the Kullback--Leibler divergence between nearby members is a
series whose $n$-th coefficient is the $n$-th Fisher tensor, and that tensor is
the connected cumulant of the whitened residual; the quadratic truncation is
exact exactly when the higher cumulants vanish~\cite{bal2026triality}
(Thm.~1, Cor.~2, App.~A). The exact cumulant reading holds only in such a
chart --- the whitened residual under the Gaussian reference is one, and the
pooled latent is one on the linear subject; off that chart the same number is a
deviation from the reference cell's own third moment and is named so. The study
reports the first such rung (the connected third cumulant of the whitened
residual, with the fourth-order companion computed but not used) as a supporting
statistic of the N/S signatures; it is not a third claim and changes no
estimand.
...
output, with the local-linear error. The capacity-matched control is the same
argument form as the paired-seed, zero-initialised-gate evidence design
of~\cite{bal2026quiver}: an augmented arm earns any gain under identical splits
against a capacity-matched baseline, so a difference is read as information
rather than capacity. \emph{Resolvability and patching}: the
...
365--411.
\bibitem{bal2026triality} A. Bal, M. Klute, B. Maier, M. Spannowsky. ``From
Information Geometry to Jet Substructure: A Triality of Cumulant Tensors, Energy
Correlators, and Hypergraphs.'' \href{https://arxiv.org/abs/2605.03063}{arXiv:2605.03063}
v2 (2026). The local KL expansion, its connected-cumulant coefficients, and the
chart caveat (Thm.~1, Cor.~2, App.~A).
\bibitem{bal2026quiver} A. Bal, M. Binder, M. Klute, B. Maier, M. Spannowsky.
``QUIVER: Quantum-Informed Views for Enhanced Representations in Large ML
Models.'' \href{https://arxiv.org/abs/2606.02785}{arXiv:2606.02785} (2026).
Cited only for its paired-seed, zero-initialised-gate evidence design.
