...
QUADRATIC_BASIS = ("in_rms", "in_kurtosis", "unc_recon_energy", "z_mahalanobis", "gr_input_maha", "gr_output_maha",
                   "gr_out_of_span", "gr_support_novelty")
...
class AlarmTimeReference:
    ...
    def fit(cls, subject: Subject, ref: ReferenceCell, X_fit: np.ndarray, geometry: Geometry, *, task_target: int = 0,
        ...
        means, seconds = {}, {}
        for hook in HOOKS:
            m, s = pooled_stage(rep, hook)
            means[hook] = HookNull.fit(m)
            if s is not None:
                seconds[hook] = HookNull.fit(s)
        z = rep["z"]
        J = np.asarray(subject.jac_output(ref.z_mean, geometry), dtype=float)
        return cls(ref=ref, hook_mean_nulls=means, hook_second_nulls=seconds, z_null=HookNull.fit(z),
                   support=SupportEstimator.fit(z, k=support_k), task=TaskMetric.one_hot(J, task_target, getattr(subject, "target_names", ())),
                   task_target=task_target, n_pcs=n_pcs)

    def to_dict(self) -> dict:
        return {"hook_mean_nulls": {h: {"dim": n.dim, "reduced_dim": n.reduced_dim, "alpha": n.alpha, "n_ref": n.n_ref, "rule": n.rule}
...
def build_alarm_time_features(subject: Subject, atref: AlarmTimeReference, inputs: AlarmTimeInputs) -> FeatureBatch:
    ...
    # capacity control: quadratic expansion of generic_rich scalars, truncated to the intermediate count
    n_im = len(INTERMEDIATE_HOOKS) * (2 + atref.n_pcs)
    basis = [b[k] for k in QUADRATIC_BASIS]
