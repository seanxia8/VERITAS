...
| NuRadioMC (arm B) | declared: frozen NuRadioReco direction/energy error (trigger efficiency secondary) | — | **PENDING** | pending |
...
5. Junjie's agreement on the title and the two-claim framing; the transfer arm (B, A or Prometheus) and its K; gates A0, D5.
6. External pre-registration venue and identifier, if any: **none yet**.
7. **D7 — pooled third-moment transform on both arms: TICKED yes (17 Sep 2026).** The whitened-residual connected third cumulant and the pooled channel third moment enter the arms on both sides; compression is declared as `n_pcs` reference-PCA coordinates plus one Frobenius-deviation scalar; the quadratic capacity control is re-truncated to the intermediate feature count so `generic_rich_matched` stays count-matched. The chart rule is recorded (`cumulant` / `deviation`).
8. **D8 — cubic task score as the registered supporting intermediate score: TICKED yes (17 Sep 2026).** `raw_task_cubic = Δ_a Δ_b Δ_c Î3_abc`, `Î3` on `reference_fit` `z` in the M_task-whitened chart, calibrated on `calibration` windows and reported beside the task length; it never enters the Claim-2 primary ΔAUROC_harm and never enters `GENERIC_COMMITTED`.
9. **D9 — R(3) hook-selection diagnostic: NOT TICKED (deferred, 17 Sep 2026).** Not implemented and not mentioned as implemented; revisit when the designed controls are available for the subject whose hooks it would choose among.
10. **D10 — frozen-propagator hypergraph subject as a candidate third subject: TICKED yes, design and interface only (17 Sep 2026).** `latent_monitor/hypergraph_subject.py` implements the `Subject` interface with P2/P3 frozen from the reference cell and Laplacian unit tests; no training, no GPU run, no result, gated on the trained Tier-1 run. It is one of two nonlinear subjects, both to be reported, neither chosen by result.
